	...
	methods: {
		...
		getChart1Options() {
			return {
				...
				grid: {
					row: {
						colors: ['rgba('+ appVariable.color.componentColorRgb + ', .1)', 'transparent'],
						opacity: 0.5
					},
				},
				markers: {
		...
		getChart3Options() {
			return {
				...
				colors: [appVariable.color.pink, appVariable.color.dark],
				xaxis: {
					type: 'datetime',
		...
		getChart4Options() {
			return {
				...
				stroke: {
					show: true,
					width: 1,
					colors: [appVariable.color.componentBg]
				},
				xaxis: {
		...
		getChart6Options() {
			return {
				...
				plotOptions: {
					candlestick: {
						colors: {
							upward: appVariable.color.teal,
							downward: appVariable.color.red
						}
					}
				}
			};
		...
		getChart7Options() {
			return {
				...
				colors: [appVariable.color.blue, appVariable.color.orange, appVariable.color.teal, appVariable.color.pink],
				fill: {
					opacity: 0.8
				},
				title: {
		...
		getChart8Options() {
			return {
				...
				colors: [appVariable.color.blue, appVariable.color.orange, appVariable.color.teal],
				xaxis: {
					tickAmount: 10,
					labels: {
						formatter: function(val) {
			...
			return {
				...
				plotOptions: {
					radar: {
						size: 140,
						polygons: {
							strokeColor: 'rgba('+ appVariable.color.componentColorRgb + ', .15)',
							fill: {
								colors: ['rgba('+ appVariable.color.componentColorRgb + ', .05)', 'rgba('+ appVariable.color.componentColorRgb + ', .15)']
							}
						}
					}
				},
