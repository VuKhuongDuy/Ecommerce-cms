<template>
  <div class="row form-create">
    <div class="col-xl-6">
      <div class="form-group mb-3">
        ...
        <input v-model="createCategory.name" type="text" class="form-control" id="username" placeholder="Your username">
      </div>
    </div>
      ...
      <div class="form-group mb-3">
        ...
        <table>
          ...
          <tr>
            ...
            <td>
              <select v-model="prop.type" class="form-control">
                <option :value="PropTypes.string">{{ PropTypesString.string }}</option>
                <option :value="PropTypes.number">{{ PropTypesString.number }}</option>
              </select>
            </td>
            <td rowspan="2">
          ...
          <tr v-if="prop.type === PropTypes.number">
            ...
            <td>
              <input v-model="prop.defaultNumber[0]" type="number" class="form-control" id="username"
                placeholder="Bắt đầu">
              <input v-model="prop.defaultNumber[1]" type="number" class="form-control" id="username"
          ...
          <tr v-else>
            ...
            <td>
              <input v-model="prop.defaultText[jndex]" type="text" class="form-control" id="username"
                placeholder="Giá trị" v-for="(text, jndex) in prop.defaultText" :key="jndex"
  ...
  <button type="button" data-bs-dismiss="modal" class="btn btn-primary pr-2"
    @click="(event) => saveCategory(event, createCategory)">Save</button>
...
export default {
  ...
  async mounted() {
    this.list_props = await Promise.all(lodash.cloneDeep(this.listProp).map(p => {
      let defaultText = []
      let defaultNumber = []
  ...
  methods: {
    async saveCategory(event, createCategory) {
      let data = []
      await Promise.all(this.list_props.map(p => {
        const propValue = lodash.cloneDeep(p)
        data.push(
          {
            name: propValue.name,
            values: propValue.type === PropTypes.string ? propValue.defaultText.filter(e => e) : propValue.defaultNumber,
    ...
    addOrDelTextProp(event, index, jindex) {
      ...
      const lengthNotNull = this.list_props[index].defaultText.filter(t => t).length
